...
* @package	copix
* @subpackage auth
* @version	$Id: CopixLdapUser.class.php,v 1.8 2005/03/07 13:18:44 gcroes Exp $
...
/**
* This is the base class for authentification process in a LDAP Directory
...
*/
class CopixLdapUser extends CopixUser {
    /**
    ...
    */
    function _doLogin ($name, $password=null){
        ...
        if ($result->{strtolower ($this->passwordField)} === $this->cryptPassword ($password)){
    ...
    */
    function getList (){
        ...
        $rs = $ct->doQuery ('(objectClass=posixaccount)');
        while ($r = $rs->fetch ()){
    ...
    */
    function findByLogin ($pattern){
        ...
        $toReturn = array ();
        $ct = CopixLDAPFactory::getConnection ();
        $rs = $ct->doQuery ('&(objectClass=posixaccount)(uid=*'.$pattern.'*)');
        while ($r = $rs->fetch ()){
            $pu = new $userobjectname ();
            foreach ($this->_fieldPropList as $field=>$userField) {
                $pu->$userField = $r->$field;
